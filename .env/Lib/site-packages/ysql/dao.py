...
from ysql.entity import _parse_constraints, is_entity, FOREIGN_KEY_PLACEHOLDER, \
...
def _generate_create_table_sql(self):
    ...
    for field_name, field_type in fields.items():
        ...
        # 跳过忽略属性
        if CONSTRAINT_TYPE_IGNORE in [constraint.type for constraint in constraints]:
            continue
        # 处理外键
        foreign_key_constraint = [constraint.value
                                  for constraint in constraints
                                  if constraint.type == CONSTRAINT_TYPE_FOREIGN_KEY]
        if len(foreign_key_constraint) == 1:
            constraint = foreign_key_constraint[0]
            foreign_key_sql = constraint.replace(FOREIGN_KEY_PLACEHOLDER, field_name)
            foreign_key_constraints.append(foreign_key_sql)

        # 处理数据类型
        datatype_constraint = [constraint.value
                               for constraint in constraints
                               if constraint.type == CONSTRAINT_TYPE_DATATYPE]
        if len(datatype_constraint) == 0:
            datatype = __convert_to_sqlite_datatype(field_type)
        elif len(datatype_constraint) == 1:
            datatype = datatype_constraint[0]
        else:
            raise TypeError(
                '数据类型指定错误：')
        ...
        comment_constraint = [constraint
                              for constraint in constraints
                              if constraint.type == CONSTRAINT_TYPE_COMMENT]
        if len(comment_constraint) == 1:
            comment_constraint = comment_constraint[0]
            comment_constraint.value = __process_comment(value=comment_constraint.value, db_type=self.db_type)

        # 处理自增主键
        auto_primary_key_constraint = [constraint
                                       for constraint in constraints
                                       if constraint.type == CONSTRAINT_TYPE_AUTO_PRIMARY_KEY]
        if len(auto_primary_key_constraint) == 1:
            auto_primary_key_constraint = auto_primary_key_constraint[0]
            auto_primary_key_constraint.value = __process_auto_primary_key(value=auto_primary_key_constraint.value,
                                                                           db_type=self.db_type)

        # 只保留约束值
        constraints = [constraint.value
                       for constraint in constraints
                       if constraint.type != CONSTRAINT_TYPE_FOREIGN_KEY and
                       constraint.type != CONSTRAINT_TYPE_DATATYPE]
        # 合并其他约束条件
        constraint = " ".join(constraints)

        # 拼接字段定义
        field_definition = f"{field_name} {datatype} {constraint}"
